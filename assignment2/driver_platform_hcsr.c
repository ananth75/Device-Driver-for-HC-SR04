...
static ssize_t distance_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	printk("distance = %d\n", pphcsr_dev_obj->pfifo_t->ptym[pphcsr_dev_obj->pfifo_t->tail].distance);
	return snprintf(buf, PAGE_SIZE, "%d\n",pphcsr_dev_obj->pfifo_t->ptym[pphcsr_dev_obj->pfifo_t->tail].distance);
...
static ssize_t trigger_pin_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    sscanf(buf, "%d", &pphcsr_dev_obj->trigger_pin);
	printk("Trigger pin has been stored\n");
	gpio_request(pphcsr_dev_obj->trigger_pin, "sysfs");    							
   	gpio_direction_output(pphcsr_dev_obj->trigger_pin, 0);
...
{
	phcsr_dev_str pphcsr_dev_obj = dev_get_drvdata(dev);
        sscanf(buf, "%d", &pphcsr_dev_obj->echo_pin);
	printk("Echo pin has been stored\n");
	gpio_request(pphcsr_dev_obj->echo_pin, "sysfs");        
	gpio_direction_input(pphcsr_dev_obj->echo_pin);
...
{
      phcsr_dev_str pphcsr_dev_obj = dev_get_drvdata(dev);
      sscanf(buf, "%d", &pphcsr_dev_obj->samples);
...
{
      phcsr_dev_str pphcsr_dev_obj = dev_get_drvdata(dev);
      sscanf(buf, "%d", &pphcsr_dev_obj->time);
...
static irqreturn_t gpio_irq_handler(int irq, void *dev_id)
{
	...
	int check = gpio_get_value(pphcsr_dev_str->echo_pin);
...
{
	phcsr_dev_str pphcsr_dev_obj= file->private_data;
	...
	gpio_set_value_cansleep(pphcsr_dev_obj->trigger_pin, 0);
	gpio_set_value_cansleep(pphcsr_dev_obj->trigger_pin, 1);
	udelay(10);
	gpio_set_value_cansleep(pphcsr_dev_obj->trigger_pin, 0);
        mdelay(200);
	r_int_config(pphcsr_dev_obj->echo_pin, (void*)pphcsr_dev_obj);
	gpio_set_value_cansleep(pphcsr_dev_obj->trigger_pin, 0);
	gpio_set_value_cansleep(pphcsr_dev_obj->trigger_pin, 1);
	udelay(10);
	gpio_set_value_cansleep(pphcsr_dev_obj->trigger_pin, 0);
...
static int hcsr_close(struct inode *inodep, struct file *file)
{
	...
	gpio_free(pshcsr_dev_obj->echo_pin); 
    	gpio_free(pshcsr_dev_obj->trigger_pin); 
...
phcsr_dev_str init_device(void* Pchip_handle)
{
	...
	phcsr_dev_str_obj->pfifo_t = kmalloc(sizeof(struct fifo), GFP_KERNEL);
	phcsr_dev_str_obj->pfifo_t->head = 0;
	phcsr_dev_str_obj->pfifo_t->tail = 0;
...
*/	
	...
	printk("inside probe  minor = %d", phcsr_dev_str_obj->dev_misc.minor);
        ...
        if (!(phcsr_dev_str_obj->pfifo_t->ptym = kmalloc(sizeof(hcsr_measure_t)* SIZE_OF_BUF, GFP_KERNEL)))
        {
                printk("Bad Kmalloc\n");
...
static int P_driver_probe(struct platform_device *dev_found)
{
	...
	struct HCSR_chip *hcsrchip;
	phcsr_dev_str phcsr_dev_obj = kmalloc(sizeof(struct HCSR_DEV_OBJ), GFP_KERNEL);
	...
	if(!(phcsr_dev_obj = init_device((void *)hcsrchip)))
	{
		printk("device initialisation failed\n");
	}
	
	printk( "Returned after initialization\n");
	INIT_LIST_HEAD(&phcsr_dev_obj->device_entry);
	printk( "About to create a device\n");
	list_add(&phcsr_dev_obj->device_entry, &device_list );
	printk( "About to create a device after adding things in the list\n");
	/* device */
	//printk("");
	printk( "Device to be created is named as %s \n",phcsr_dev_obj->dev_name); 
        hcsr_device = device_create(hcsr_class, NULL, hcsr_dev, phcsr_dev_obj, phcsr_dev_obj->dev_name);
	...
	ret = device_create_file(hcsr_device, &dev_attr_trigger_pin);
        if (ret < 0) {
                printk(" cant create device attribute %s %s\n", 
                       phcsr_dev_obj->dev_name, dev_attr_trigger_pin.attr.name);
        }
	
	ret = device_create_file(hcsr_device, &dev_attr_echo_pin);
        if (ret < 0) {
               printk(" cant create device attribute %s %s\n", 
                       phcsr_dev_obj->dev_name, dev_attr_echo_pin.attr.name);
        }

	rval = device_create_file(hcsr_device, &dev_attr_samples);
        if (rval < 0) {
               printk(" cant create device attribute %s %s\n", 
                       phcsr_dev_obj->dev_name, dev_attr_samples.attr.name);
        }

	rval = device_create_file(hcsr_device, &dev_attr_sampling_period);
        if (rval < 0) {
       printk(" cant create device attribute %s %s\n", 
                       phcsr_dev_obj->dev_name, dev_attr_sampling_period.attr.name);
        }
/*
	rval = device_create_file(hcsr_device, &dev_attr_Enable);
        if (rval < 0) {
      printk(" cant create device attribute %s %s\n", 
                       phcsr_dev_obj->dev_name, dev_attr_Enable.attr.name);
        }
*/
        rval = device_create_file(hcsr_device, &dev_attr_distance);
        if (rval < 0) {
   	 printk(" cant create device attribute %s %s\n", 
                       phcsr_dev_obj->dev_name, dev_attr_distance.attr.name);
        }
printk("probe done\n");
...
static int P_driver_remove(struct platform_device *pdev)
{
	...
	list_for_each_entry(ptempcontext, &device_list, device_entry)
	{
		...
		kfree(ptempcontext->pfifo_t->ptym);
   	    printk("freed ptym\n");
	    kfree(ptempcontext->pfifo_t);
   	    printk("freed buf\n");
